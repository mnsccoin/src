...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected mnsc and zmnsc supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zmnscsupply']:
                assert_equal(node['zmnscsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check mnsc and zmnsc supply at the beginning
        # ------------------------------------------
        # zmnsc supply: 2 coins for each denomination
        ...
        # mnsc supply: block rewards minus burned fees for minting
        ...
        # Verify that mnsc and zmnsc supplies were properly updated after the spends and reorgs
        self.log.info("Check mnsc and zmnsc supply...")
