...
CzMNSCWallet::CzMNSCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmnsc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMNSCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMNSCSeed_deprecated()) {
                    LogPrintf("%s: Updated zMNSC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmnsc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmnsc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMNSCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMNSCCount(nCountLastUsed);
    else if (!walletdb.ReadZMNSCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMNSCWallet::Lock()
{
    seedMaster = 0;
}

void CzMNSCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMNSCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMNSC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMNSCWallet::LoadMintPoolFromDB()
{
...
void CzMNSCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMNSCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMNSCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmnscTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMNSCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMNSC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmnscTracker which also adds to database
    pwalletMain->zmnscTracker->Add(dMint, true);
...
void CzMNSCWallet::SeedToZMNSC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMNSCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMNSCWallet::GenerateDeterministicZMNSC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMNSCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMNSCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMNSCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
