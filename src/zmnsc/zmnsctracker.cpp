// Copyright (c) 2018-2019 The MNSAVINGS developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zmnsc/deterministicmint.h>
#include "zmnsctracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzMNSCTracker::CzMNSCTracker(std::string strWalletFile)
{
...
CzMNSCTracker::~CzMNSCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMNSCTracker::Init()
{
...
bool CzMNSCTracker::Archive(CMintMeta& meta)
{
...
bool CzMNSCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMNSCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMNSCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMNSCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzMNSCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzMNSCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzMNSCTracker::GetSerialHashes()
{
...
CAmount CzMNSCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMNSCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMNSCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMNSCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMNSCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMNSCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMNSCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMNSCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMNSCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMNSCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMNSCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMNSCWallet* zMNSCWallet)
{
    bool iszMNSCWalletInitialized = (NULL != zMNSCWallet);
    CMintMeta meta;
    ...
    if (! iszMNSCWalletInitialized)
        zMNSCWallet = new CzMNSCWallet(strWalletFile);
    meta.isSeedCorrect = zMNSCWallet->CheckSeed(dMint);
    if (! iszMNSCWalletInitialized)
        delete zMNSCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMNSCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMNSCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMNSCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMNSCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMNSCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMNSCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMNSCWallet* zMNSCWallet = new CzMNSCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMNSCWallet);
        }
        delete zMNSCWallet;
        LogPrint("zero", "%s: added %d dzmnsc from DB\n", __func__, listDeterministicDB.size());
    }
