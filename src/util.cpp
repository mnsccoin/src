...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "mnsavings" is a composite category enabling all MNSAVINGS-related debug output
            if (ptrCategory->count(std::string("mnsavings"))) {
...
#else
    const char* pszModule = "mnsavings";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MNSAVINGS
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MNSAVINGS
// Mac: ~/Library/Application Support/MNSAVINGS
// Unix: ~/.mnsavings
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MNSAVINGS";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "mnsavings.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty mnsavings.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override mnsavings.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "mnsavingsd.pid"));
