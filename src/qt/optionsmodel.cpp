...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MNSC);
    ...
    if (settings.contains("nAnonymizemnsavingsAmount") || reset)
        SoftSetArg("-anonymizemnsavingsamount", settings.value("nAnonymizemnsavingsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mnsavings.cpp during shotdown to also remove the window positions
