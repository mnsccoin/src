...
#include "qt/mnsavings/forms/ui_addresseswidget.h"
#include "qt/mnsavings/addresslabelrow.h"
#include "qt/mnsavings/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(MNSAVINGSGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a MNSAVINGS address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress mnscAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(mnscAdd.Get(), label.toUtf8().constData(),
                mnscAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
