...
#include "qt/mnsavings/topbar.h"
#include "qt/mnsavings/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/mnsavings/lockunlock.h"
#include "qt/mnsavings/qtutils.h"
#include "qt/mnsavings/receivedialog.h"
...
TopBar::TopBar(MNSAVINGSGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopmnsc, ui->labelAmountTopzmnsc}, "amount-small-topbar");
    setCssProperty({ui->labelAmountmnsc, ui->labelAmountzmnsc}, "amount-topbar");
    setCssProperty({ui->labelPendingmnsc, ui->labelPendingzmnsc, ui->labelImmaturemnsc, ui->labelImmaturezmnsc}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzmnsc->setVisible(false);
    ui->labelAmountzmnsc->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzmnsc->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezmnsc->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("MNSAVINGS")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount mnscAvailableBalance = balance + delegatedBalance;

    // zMNSC Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalmnsc = GUIUtil::formatBalance(mnscAvailableBalance, nDisplayUnit);
    QString totalzmnsc = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopmnsc->setText(totalmnsc);
    ui->labelAmountTopzmnsc->setText(totalzmnsc);

    // Expanded
    ui->labelAmountmnsc->setText(totalmnsc);
    ui->labelAmountzmnsc->setText(totalzmnsc);

    ui->labelPendingmnsc->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzmnsc->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturemnsc->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezmnsc->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
