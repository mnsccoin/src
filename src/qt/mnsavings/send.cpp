...
#include "qt/mnsavings/send.h"
#include "qt/mnsavings/forms/ui_send.h"
#include "qt/mnsavings/addnewcontactdialog.h"
#include "qt/mnsavings/qtutils.h"
#include "qt/mnsavings/sendchangeaddressdialog.h"
#include "qt/mnsavings/optionbutton.h"
#include "qt/mnsavings/sendconfirmdialog.h"
#include "qt/mnsavings/myaddressrow.h"
#include "qt/mnsavings/guitransactionsutils.h"
...
SendWidget::SendWidget(MNSAVINGSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("MNSC");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zMNSC");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (MNSC)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a MNSAVINGS address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 MNSC");
    ...
    setCssProperty(coinIcon, "coin-icon-mnsc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onMNSCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onMNSCSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send MNSC");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zMNSC");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZmnsc = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZmnsc));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZmnsc ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendmnsc = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendmnsc ? AskPassphraseDialog::Context::Send_MNSC : AskPassphraseDialog::Context::Send_zMNSC, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendmnsc) ? send(recipients) : sendZmnsc(recipients)) {
...
bool SendWidget::sendZmnsc(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zMNSC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zMNSC selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZmnscControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZmnscControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZmnsc(
        ...
        inform(tr("zMNSC transaction sent!"));
        ZmnscControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZMNSC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zMNSC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zMNSC transaction
        ...
        emit message("zMNSC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isMNSC){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any MNSC to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZmnscControlDialog *zmnscControl = new ZmnscControlDialog(this);
            zmnscControl->setModel(walletModel);
            zmnscControl->exec();
            ui->btnCoinControl->setActive(!ZmnscControlDialog::setSelectedMints.empty());
            zmnscControl->deleteLater();
        } else {
            inform(tr("You don't have any zMNSC in your balance to select."));
        }
    }
}
...
void SendWidget::onMNSCSelected(bool _isMNSC){
    isMNSC = _isMNSC;
    setCssProperty(coinIcon, _isMNSC ? "coin-icon-mnsc" : "coin-icon-zmnsc");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress mnscAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(mnscAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(mnscAdd.Get(), dialog->getLabel().toStdString(), "send")) {
