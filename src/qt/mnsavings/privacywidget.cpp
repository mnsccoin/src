...
#include "qt/mnsavings/privacywidget.h"
#include "qt/mnsavings/forms/ui_privacywidget.h"
#include "qt/mnsavings/qtutils.h"
#include "guiutil.h"
#include "qt/mnsavings/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(MNSAVINGSGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zMNSC anonymizes your MNSC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zMNSC or convert back to MNSC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 MNSC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zMNSC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zMNSC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zMNSC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zMNSC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zMNSC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zMNSC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zMNSC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zMNSC");
    ...
    // Only Convert to MNSC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzMNSC->setTitleClassAndText("btn-title-grey", "Total 0 zMNSC");
    ui->btnTotalzMNSC->setSubTitleClassAndText("text-subtitle", "Show denominations of zMNSC owned.");
    ui->btnTotalzMNSC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select MNSC outputs to mint into zMNSC.");
    ...
    connect(ui->btnTotalzMNSC, SIGNAL(clicked()), this, SLOT(onTotalZmnscClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zMNSC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of MNSC to mint into zMNSC"));
    }else{
        btnText = tr("Convert back to MNSC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zMNSC to convert back into MNSC"));
    }
...
void PrivacyWidget::onTotalZmnscClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzMNSC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzMNSC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zMNSC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zMNSC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zMNSC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zMNSC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZmnsc(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zMNSC converted back to MNSC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMNSC </b>";
    ...
    ui->btnTotalzMNSC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
