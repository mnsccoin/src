...
#include "qt/mnsavings/dashboardwidget.h"
#include "qt/mnsavings/forms/ui_dashboardwidget.h"
#include "qt/mnsavings/sendconfirmdialog.h"
#include "qt/mnsavings/txrow.h"
...
DashboardWidget::DashboardWidget(MNSAVINGSGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of MNSC staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquaremnsc, "square-chart-mnsc");
    setCssProperty(ui->labelSquarezmnsc, "square-chart-zmnsc");
    setCssProperty(ui->labelmnsc, "text-chart-mnsc");
    setCssProperty(ui->labelZmnsc, "text-chart-zmnsc");
    ui->labelZmnsc->setVisible(false);
    ui->labelSquarezmnsc->setVisible(false);
    ...
    ui->labelAmountZmnsc->setText("0 zMNSC");
    ui->labelAmountZmnsc->setVisible(false);
    ui->labelAmountmnsc->setText("0 MNSC");
    setCssProperty(ui->labelAmountmnsc, "text-stake-mnsc-disable");
    setCssProperty(ui->labelAmountZmnsc, "text-stake-zmnsc-disable");
    ...
    ui->btnHowTo->setText(tr("How to get MNSC"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("MNSC")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool ismnsc = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZMNSC;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair MNSC, zMNSC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            mnsc = (pair.first != 0) ? pair.first / 100000000 : 0;
            zmnsc = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalmnsc += pair.first;
            chartData->totalZmnsc += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesmnsc.append(mnsc);
        chartData->valueszmnsc.append(zmnsc);

        int max = std::max(mnsc, zmnsc);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("MNSC");
    set1 = new QBarSet("zMNSC");
    ...
    set0->append(chartData->valuesmnsc);
    set1->append(chartData->valueszmnsc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalmnsc > 0 || chartData->totalZmnsc > 0) {
        setCssProperty(ui->labelAmountmnsc, "text-stake-mnsc");
        setCssProperty(ui->labelAmountZmnsc, "text-stake-zmnsc");
    } else {
        setCssProperty(ui->labelAmountmnsc, "text-stake-mnsc-disable");
        setCssProperty(ui->labelAmountZmnsc, "text-stake-zmnsc-disable");
    }
    forceUpdateStyle({ui->labelAmountmnsc, ui->labelAmountZmnsc});
    ui->labelAmountmnsc->setText(GUIUtil::formatBalance(chartData->totalmnsc, nDisplayUnit));
    ui->labelAmountZmnsc->setText(GUIUtil::formatBalance(chartData->totalZmnsc, nDisplayUnit, true));
