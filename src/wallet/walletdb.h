...
public:
    ...
    bool WriteZMNSCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZMNSCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZMNSCSeed_deprecated(uint256& seed);
    bool EraseZMNSCSeed();
    bool EraseZMNSCSeed_deprecated();

    bool WriteZMNSCCount(const uint32_t& nCount);
    bool ReadZMNSCCount(uint32_t& nCount);
