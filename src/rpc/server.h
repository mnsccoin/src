...
/**
 * MNSAVINGS RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZmnscSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzmnscseed(const UniValue& params, bool fHelp);
extern UniValue getzmnscseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzmnsc(const UniValue& params, bool fHelp);
extern UniValue dzmnscstate(const UniValue& params, bool fHelp);
