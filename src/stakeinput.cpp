...
CZmnscStake::CZmnscStake(const libzerocoin::CoinSpend& spend)
{
...
int CZmnscStake::GetChecksumHeightFromMint()
{
...
int CZmnscStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZmnscStake::GetChecksum()
{
    return nChecksum;
}

// The zMNSC block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZmnscStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZmnscStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zMNSC staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZmnscStake::GetUniqueness()
{
    //The unique identifier for a zMNSC is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZmnscStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZmnscStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zMNSC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMNSCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMNSC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMNSC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMNSCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMNSC output", __func__);
...
bool CZmnscStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZmnscStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzMNSCTracker* zmnscTracker = pwallet->zmnscTracker.get();
    CMintMeta meta;
    if (!zmnscTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmnscTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!MNSC Stake
bool CmnscStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CmnscStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CmnscStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CmnscStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CmnscStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CmnscStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CmnscStake::GetUniqueness()
{
    //The unique identifier for a MNSC stake is the outpoint
...
CBlockIndex* CmnscStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
